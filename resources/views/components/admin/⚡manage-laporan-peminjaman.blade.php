...
new #[Title('Kelola Laporan Peminjaman')] class extends Component {
    ...
    {
        return LaporanPeminjaman::when($this->search, fn($q) => $q->where('tahun', 'like', "%{$this->search}%"))->latest()->paginate($this->perPage);
    }
    ...
    public function deleteLaporan(LaporanPeminjaman $laporan)
    {
        try {
        ...
        <div
            ...
            <div class="flex items-center justify-between border-b border-stone-100 py-2 dark:border-stone-800">
                <span class="font-mono text-[10px] uppercase tracking-wider text-stone-400 dark:text-stone-500">
                    {{ $this->laporanPeminjaman->total() }} {{ __('pengguna ditemukan') }}
                </span>
                ...
                <table class="w-full border-collapse text-left text-[11px]">
                    <thead>
                        <tr
                            ...
                            <th class="px-2.5 py-1.5 sm:table-cell">{{ __('Laporan') }}</th>
                            <th class="px-2.5 py-1.5 sm:table-cell">{{ __('Tahun') }}</th>
                            <th class="px-2.5 py-1.5 md:table-cell">{{ __('Total Hibah') }}</th>
                            <th class="w-20 px-2.5 py-1.5 text-right">{{ __('Aksi') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-stone-100 dark:divide-stone-800/60">
                        @forelse ($this->laporanPeminjaman as $laporan)
                            <tr wire:key="laporan-{{ $laporan->id_user }}"
                                ...
                                <td class="px-2.5 py-1.5 text-center font-mono text-stone-400 dark:text-stone-600">
                                    {{ $loop->iteration + ($this->laporanPeminjaman->currentPage() - 1) * $this->laporanPeminjaman->perPage() }}
                                </td>
                                ...
                                {{-- Total Hibah --}}
                                <td
                                    class="whitespace-nowrap px-2.5 py-1.5 text-stone-500 md:table-cell dark:text-stone-400">
                                    {{ $laporan->hibahRupiah }}
                                </td>
                            ...
                            <tr>
                                <td colspan="9" class="px-2.5 py-10 text-center">
                                    <div class="flex flex-col items-center gap-1.5 text-stone-400">
                                        <p class="text-xs font-medium">{{ __('Tidak ada Laporan ditemukan') }}</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="border-t border-stone-100 px-4 py-3 dark:border-stone-800">
                {{ $this->laporanPeminjaman->links('pagination::tailwind') }}
            </div>
        </div>
