...
new class extends Component {
    ...
    public function editKoordinator(User $userId)
    {
        $this->user = $userId;
    ...
    public function updateKoordinator()
    {
        try {
            $this->form->update();
            session()->flash('success', 'Informasi Koordinator berhasil diperbarui.');
            $this->dispatch('update-success', ['message' => 'Informasi koordinator berhasil diperbarui.']);
...
<div>
    <div x-data="{
        ...
        idPeminjaman: null,
        init() {
            window.addEventListener('edit-koordinator-modal', (e) => {
                this.message = '';
                $wire.editKoordinator(e.detail.userId);
        ...
        <div class="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg bg-white shadow-xl dark:bg-stone-900"
            @click.stop>
            <form wire:submit.prevent="updateKoordinator" class="relative space-y-3 p-4">

                {{-- Header Modal --}}
                <div>
                    <h3 class="text-sm font-semibold text-stone-800 dark:text-stone-100">
                        {{ __('Ubah Informasi Koordinator') }}
                    </h3>
                ...
                <div class="relative">
                    ...
                    {{-- Indikator Loading khusus saat method create berjalan --}}
                    <div wire:loading wire:target="updateKoordinator"
                        class="absolute inset-0 z-50 flex items-center justify-center rounded-md bg-white/60 backdrop-blur-[0.5px] dark:bg-stone-900/60">
                        <div
                            class="flex items-center gap-1.5 rounded-md border border-stone-100 bg-white px-2.5 py-1.5 shadow-sm dark:border-stone-700 dark:bg-stone-800">
                            <svg class="text-sage-600 dark:text-sage-400 h-3.5 w-3.5 animate-spin" fill="none"
